# Build Roadmap

Tracking progress section by section.

- [x] 1. Repo scaffold + architecture doc
- [x] 2. Kernel core — TCB, ready lists, priority scheduler
- [ ] 3. Context switch — PendSV/SysTick in ARM assembly
- [ ] 4. Sync primitives — mutex w/ priority inheritance, semaphore, queue
- [ ] 5. Software timers + tick management
- [ ] 6. Port layer — linker script, startup code, STM32F4 register access
- [ ] 7. Application demo — power-supply state machine on nimbleRTOS
- [ ] 8. Host-side unit tests + GitHub Actions CI
- [ ] 9. README polish, license, push to GitHub
